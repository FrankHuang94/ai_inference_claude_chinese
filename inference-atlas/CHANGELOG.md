...
### 模块 13 — 第二批：投机解码与量化压缩（5/12）

**状态**：模块 13 `进行中`（5/12 篇）。全库正文 **841,878 字**，124 篇文档，181 图，0 死链。

#### 新增文档（2 篇）

| 文档 | 要点 |
|---|---|
| `04_speculative_decoding_papers.md` | 7 张卡片（P-005、P-016~P-021）；**演化线索图**（框架 → 树 → 自投机 → 提高接受率 → 去掉训练）；**三路线成本表**（模型 / 训练 / 算力，三种成本各推到一个角）；全族**共同失效条件**：算力饱和后一起失效，换论文救不了 |
| `05_quantization_and_compression_papers.md` | 7 张卡片（P-022~P-028）；**W-only vs W+A 分野图**（前者省带宽不省算力）；离群值的三种策略对照（隔离 / 搬走 / 绕开）；有效位宽 $b_w + b_s/g$ 与「不说组大小的 2 bit 没有信息量」 |

#### 数据

新增 P-016~P-028 共 13 条，`citation_status` 一律 `待核实`；
`references.bib` 同步至 28 条，双向一致。
**本批 5 个 arXiv 编号（EAGLE、EAGLE-2、SmoothQuant、AWQ、SqueezeLLM）逐条检索核对后才写入**——
AGENTS.md 第 11 节「禁止」第 3 条明令不得凭记忆填写此类标识。

SpecInfer（P-020）的**完整作者名单与官方实现仓库均未能核实**，
已如实标为 `待核实` 并在卡片正文说明，而非用一个看起来合理的仓库链接搪塞。

### 模块 13 — 第三批：编译器/kernel 与分布式/MoE（7/12）

**状态**：模块 13 `进行中`（7/12 篇）。全库正文 **855,797 字**，126 篇文档，183 图，0 死链。

#### 新增文档（2 篇）

| 文档 | 要点 |
|---|---|
| `06_compiler_runtime_and_kernel_papers.md` | 5 张卡片（P-029~P-033）；**「降低的是哪种成本」对照表**——只有 TVM/Ansor 直接产出更快的代码，Triton 降低的是人写 kernel 的成本、MLIR 是造编译器的成本、PyTorch 2 是用上编译器的成本；**厂商库/自动编译/手写三条路线并存，按算子挑** |
| `07_distributed_and_moe_inference_papers.md` | 6 张卡片（P-034~P-039）；核心产出是**「训练结论到推理」的迁移检查表**，逐条追问「它在训练里成立的理由，在 decode 阶段还在不在」 |

#### 第 07 章的一个前置警告

本章六篇**全部是训练论文**。它们的收益论证建立在训练的计算/通信比之上，
而 decode 每步只推进一个 token，计算量小了几个数量级、通信量却没有同比缩小。
因此本章把每张卡片的「适用边界」统一写成上述迁移问题，并给出结论：
**机制可以直接用，结论一条都不能直接搬。**

由此得到四条可直接使用的判断：张量并行不该跨节点；
流水线并行提高吞吐但不降低单请求延迟；MoE 省算力不省显存；
MoE 服务必须观测线上路由分布——容量因子按训练期整定，
而推理中丢 token 是**静默**的质量损失，不可补偿也不报错。

#### 数据

新增 P-029~P-039 共 11 条，`citation_status` 一律 `待核实`；
`references.bib` 同步至 39 条，双向一致。
本批仅 Megatron-LM 有作者提供的 BibTeX，其余书目字段全部经检索逐条核对。
其中 MLIR 有一篇**标题不同**的 arXiv 预印本（arXiv:2002.11054），
已在卡片与 bib `note` 中标明不可互相替代；
Triton、MLIR、GPipe、Switch 改用会议/期刊官方页面而非 arXiv 链接。
PyTorch 2 与 SpecInfer 的完整作者名单未能核实，已如实标注。

#### 顺带修正的两处陈旧统计

`INDEX.md` 的**合计行**（792,898 字 / 2068 表 / 176 图）与顶部日期停留在本 session 之前，
已按各模块行重算为 843,106 字 / 2146 表 / 183 图，日期更新至 2026-08-10。

### 模块 13 — 第四批：硬件/体系结构与网络/数据中心（9/12）

**状态**：模块 13 `进行中`（9/12 篇）。全库正文 **869,180 字**，128 篇文档，185 图，0 死链。

#### 新增文档（2 篇）

| 文档 | 要点 |
|---|---|
| `08_hardware_and_architecture_papers.md` | 5 张卡片（P-040~P-044）；核心产出是**「框架仍活、数字已死」对照表**——Roofline、TPU v1、Eyeriss 的数字都已过期，但它们的框架仍是本库的分析基础 |
| `09_networking_and_datacenter_papers.md` | 5 张卡片（P-045~P-049）；核心产出是**「通用数据中心形态 → 推理机群形态」对照表**，逐条说明同一问题为何在推理侧被放大 |

#### 第 08 章：一处刻意的空白

本章**一个芯片规格数字都没有给**——峰值算力、片上存储容量、功耗、能效比，
按 AGENTS.md 第 11 节「禁止」第 3 条均属不得凭记忆或二级来源填写的内容，
而本环境无法访问任何厂商技术文档。这是刻意为之，已在章首声明。

三条可直接使用的判断：decode 是带宽受限的，
$I_{\text{decode}} \approx 2/b_w$ 与机器平衡点差两个数量级，**这是算出来的不是猜的**；
$I^{*}$ 随代际上升，所以「换新卡就能变快」在带宽受限区不成立；
**LLM decode 的权重复用度约等于 1**，CNN 加速器的数据流经验在此几乎失效——
这解释了端侧 NPU 标称算力与实际 LLM 表现之间的落差。

#### 第 09 章：五篇都不是为 AI 写的

它们出自通用数据中心的建设经验，但推理机群把每一个问题都放大了：
集合通信使尾延迟放大式里的 $n$ 变成 $2L$，且**对冲请求这类缓解手段在同步通信里全部失效**；
all-to-all 是度为 $P-1$ 的天然 incast，最易触发 PFC；
而 **RDMA 网络的故障模式是「停住」不是「变慢」**——
表现为整个并行组同时无响应，监控上极易误判为计算侧问题，
因此流控计数器必须进第一轮排查。

#### 数据

新增 P-040~P-049 共 10 条，`citation_status` 一律 `待核实`；
`references.bib` 同步至 49 条，双向一致。
**本批全部为检索核对的书目**——这一族论文基本没有配套代码仓库，
没有作者提供的 BibTeX 可用。
其中 TPU v1 的 arXiv 编号因首次检索时被我在查询里预先给出（有确认偏差风险），
另用一次**不预设编号**的检索独立复核过。

### 模块 13 — 第五批：边缘推理、可靠性与基准、阅读追踪（**12/12 完成**）

**状态**：模块 13 `已完成`（12/12 篇，78,943 字，131 表，11 图）。
全库正文 **884,532 字**，131 篇文档，187 图，0 死链。

#### 新增文档（3 篇）

| 文档 | 要点 |
|---|---|
| `10_edge_inference_papers.md` | 5 张卡片（P-050~P-054）；核心判断是**三组方法会成组失效**——组一需训练侧介入，组二组三共享「激活稀疏」这一前提，而它由激活函数决定，同样不在部署方手里 |
| `11_reliability_and_benchmarking_papers.md` | 5 张卡片（P-055~P-059）；核心产出是**「静默失效全部是灰度故障」**这一统一解释，以及「无损优化只测性能、有损优化必测质量」的验收判据 |
| `12_recent_reading_tracker.md` | 不写成「我读了什么」，写成**欠账台账**：核验状态、覆盖度逐条对照、结构性缺口、解除阻塞后的三步补救流程，以及一份自我评价 |

#### 第 12 章：把阅读追踪写成欠账台账

模块 README 把它叫「近期阅读追踪」，但本模块一篇正文都没读过，
写成阅读记录是自欺。因此改写为台账，逐项记录：

- 59 条录入中 `已核验` **0** 条，`待核实` **59** 条；
  类别 A（作者仓库 BibTeX）20 条、类别 B（检索书目）39 条；
  `official_code_url` 为可读仓库 31 条、`待核实` 28 条；作者名单不完整 3 条；
- **结构性缺口**（TensorRT-LLM、HBM 器件规格、芯片算力功耗）——
  其权威来源本身不是论文，不该用「找一篇差不多的论文」填上；
- **可补而未补**（agent 推理、test-time compute）——非受阻，是卡片预算用尽；
- **解除阻塞后的三步流程**：先补 59 条存量的 `已核验`，
  再补 118 处「实验设置/关键结果」空账，最后复核本模块 5 处高风险的独立推导。

其中一处高风险推导是本模块内部的张力：第 08 章说 decode 权重复用度约为 1，
而第 04 章的树形投机验证正是靠提高每次前向的 token 数来提高复用度——
两者不矛盾（前者指朴素 decode），但已记入台账要求读原文时确认边界条件。

#### 数据

新增 P-050~P-059 共 10 条，`citation_status` 一律 `待核实`；
`references.bib` 同步至 59 条，双向一致。
本批类别 A 三条（PowerInfer、HELM、MT-Bench）来自作者仓库 BibTeX；
其余 arXiv 编号一律用限定 `arxiv.org` 域的检索确认。

**写台账时发现并更正了自己的一处错误**：初稿凭印象写下
「类别 A 14 条 / 类别 B 45 条 / 有仓库 22 条」，
逐条统计后实为 **20 / 39 / 31**，已改正。

#### 模块 13 完成情况

写作要求第 2 条的 24 项主题：20 项完成、2 项部分完成（结构性缺口）、
2 项未建卡（agent 推理与 test-time compute，缺口已记入台账）。
