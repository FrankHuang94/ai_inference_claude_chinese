...
| 上下文稀疏 | contextual sparsity | 稀疏模式是**输入的函数**而非权重的固定属性 | — |
| 冷热神经元 | hot/cold neurons | 按离线统计的激活频率划分的神经元集合 | — |
| 有效读取带宽 | effective read bandwidth | 计入访问粒度折扣后的实际带宽；闪存上与标称可差一个数量级 | GB/s |
| 存储压缩比 | storage compression ratio | 模型体积的缩小倍数；**与加速比不是同一个量** | 无量纲 |
| 场景 | scenario | 评测中规定的请求到达模式与时延约束；**跨场景数字不可比** | — |
| 封闭组 / 开放组 | closed / open division | 前者限定模型与精度保证可比，后者允许优化但需说明改动 | — |
| Pareto 前沿 | Pareto frontier | 时延与利用率之间不可同时改善的配置集合；报前沿优于报单点 | — |
| 模型评判 | LLM-as-a-judge | 用强模型代替人工做质量比较；偏差是系统性的、可部分校正 | — |
| 自我偏好 | self-preference bias | 裁判模型系统性高估与自身同族的输出 | — |
| 灰度故障 | gray failure | 组件未停机、检测通过，但应用确实受损的故障 | — |
| 差分可观测性 | differential observability | 故障检测器视角与应用视角对同一系统的观察不一致 | — |
| 欠账台账 | debt ledger | 显式记录未完成项而非隐藏它们的文档形式 | 条 |
