...
### 「待核实」是可见欠账，不是待消除的报错（2026-08-10 确立）

写作模块 13 时发现 `data/papers.csv` 中 P-001~P-005 标为
`citation_status=已核验`、`last_verified_date=2026-07-29`，
而**本节同一天**的探测表把 `arxiv.org`、`www.usenix.org` 记为「拒绝」。
两者不可能同时为真——这五条已全部降级为 `待核实`。

事后复盘，根因不只是录入疏忽，还有一条**结构性诱因**：
`scripts/paper_citation_audit.py` 原先把 `citation_status=待核实` 判为**阻断性错误**。
在一手来源不可达的环境里，这条规则让「让门禁变绿」的唯一途径变成**谎报 `已核验`**——
审计规则本身在奖励造假。

因此确立以下规则，适用于所有 CSV 与审计脚本：

1. **凡是 schema 的合法取值，审计不得判为错误**。
   `待核实` / `未公开` 是本库承认的合法状态，它们记录的是**事实**，不是格式缺陷。
2. **欠账必须可见，但不阻断**：审计应单独计数并逐条列出，写进 `reports/`，
   让欠账在报告里一眼可查；门禁只拦真错误（枚举越界、日期格式、双向不一致、缺链接）。
3. **不得为了通过门禁而修改事实字段**。若门禁与事实冲突，改门禁，不改事实。
4. **`未公开` 需要证据**。它的定义是「**确认**厂商未披露」；
   仅仅是本库没查到，应填 `待核实`。
